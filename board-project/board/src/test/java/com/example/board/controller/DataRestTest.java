...
public class DataRestTest {
  ...
  // When & Then
  mvc.perform(get("/api/articles")).andExpect(status().isOk()).andExpect(content().contentType(
  ...
  void givenNothing_whenRequestingArticle_thenReturnsArticlesJsonResponse() throws Exception {
    ...
    // When & Then
    mvc.perform(get("/api/articles/1")).andExpect(status().isOk()).andExpect(content().contentType(
        MediaType.valueOf("application/hal+json")));
  }
  ...
  void givenNothing_whenRequestingCommentsFromArticle_thenReturnsArticleCommentsJsonResponse() throws Exception {
    ...
    // When & Then
    mvc.perform(get("/api/articleComments")).andExpect(status().isOk()).andExpect(content().contentType(
        MediaType.valueOf("application/hal+json")));
  }
