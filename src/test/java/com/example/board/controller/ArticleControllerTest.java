...
public class ArticleControllerTest {
  ...
  public void givenHashtag_whenRequestingArticleHashtagSearchView_thenReturnsArticleHashtagSearchView()
    ...
    List<String> hashtags = List.of("#java", "#spring", "#boot");
    given(articleService.searchArticlesViaHashtag(eq(searchValue), any(Pageable.class))).willReturn(
        Page.empty());
    given(articleService.getHashtags()).willReturn(hashtags);
    given(paginationService.getPaginationBarNumbers(anyInt(), anyInt())).willReturn(
        List.of(1, 2, 3, 4, 5));
  ...
  @Test
  void givenNothing_whenRequestingUpdatingPage_thenRedirectsToLoginPage() throws Exception {
    // Given
    long articleId = 1L;
